...
struct semaphore {
    int count;
    struct PCB* Sem_Q, *sem_tail;
};

struct semaphore forks[5], *doorman;

void sem_wait(struct semaphore* sem, struct PCB* current) {
    printf("Wait called\n");
    sem->count--;
    ...
    if (sem->count < 0) {
        if (sem->Sem_Q == 0) {
            sem->Sem_Q = current;
            sem->sem_tail = current;
            if (head->next_PCB != 0)
                head = head->next_PCB;
        } else {
            sem->sem_tail->next_PCB = current;
            sem->sem_tail = current;
            if (head->next_PCB != 0)
                head = head->next_PCB;
        }
...
void sem_signal(struct semaphore* sem) {
    ...
    if (sem->count <= 0) {
        if (sem->Sem_Q != 0) {
            tail->next_PCB = sem->Sem_Q;
            tail = sem->Sem_Q;
            sem->Sem_Q = sem->Sem_Q->next_PCB;
        }
    }
}
...
//0: wait, 1: signal, 2: PID
void OP36(char* regs, struct PCB *current) {
    char* RX1 = (char *) malloc(2 * sizeof(char)), *RX2 = (char *) malloc(2 * sizeof(char));
    int number = 0, pid = 0;

    memcpy(RX1, regs, 2 * sizeof(char));
    memcpy(RX2, &regs[2], 2 * sizeof(char));

    switch(regHelper(RX1)) {
        case 0:
            number = R0;
            break;
        case 1:
    ...
    if (number == 0) {
        if (pid == 0) {
            sem_wait(&forks[ACC], current);
        } else {
            sem_wait(doorman, current);
        }
    } else if (number == 1) {
        if (pid == 0) {
            sem_signal(&forks[ACC]);
        } else {
            sem_signal(doorman);
        }
    }
...
int main(int argc, char* argv[]) {
    ...
    //init instruction counter from argv
    int ic = atoi(*(++argv));
    ...
    head = (struct PCB*) malloc(sizeof(struct PCB)), tail = (struct PCB*) malloc(sizeof(struct PCB));

    //For every 100 lines of memory times the number of programs inputted, set up a process control block
    //need to ignore the path and ic so argc - 2
    for(int i = 0; i < 100 * (argc - 2); i += 100) {
    ...
    //argc - 2 to account for the file path and ic
    for(int i = 0; i < 100 * (argc - 2); i++) {
